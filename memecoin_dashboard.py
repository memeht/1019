...
def sichere_suche(client, query, **kwargs):
    while True:
        try:
            return client.search_recent_tweets(query=query, **kwargs)
        except tweepy.TooManyRequests:
            print("Rate limit erreicht. Warte 15 Sekunden...")
            time.sleep(15)

def finde_neue_memecoins(max_results=100):
    query = "#crypto OR #memecoin lang:en -is:retweet"
    tweets = sichere_suche(client, query=query, tweet_fields=['entities'], max_results=max_results)
    neue_coins = set()
    if tweets.data:
...
def coin_engagement(coin, max_results=50):
    query = f"#{coin} lang:en -is:retweet"
    tweets = sichere_suche(client, query=query, tweet_fields=['public_metrics'], max_results=max_results)
...
def dashboard():
    ...
    <body>
        ...
        <p>Automatische Aktualisierung alle 60 Sekunden</p>
        {table_html}
    </body>
    </html>
    """
